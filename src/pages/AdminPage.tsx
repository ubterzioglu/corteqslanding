...
const AdminPage = () => {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (session) void fetchSubmissions();
  ...
  const exportCSV = () => {
    const headers = ["Tarih", "Tür", "Kategori", "Ad Soyad", "Ülke", "Şehir", "İşletme", "İştigal", "E-posta", "Telefon", "Açıklama", "Yarışma", "LinkedIn", "Instagram", "TikTok", "Facebook", "Twitter", "Website"];
          ...
          <div className="overflow-x-auto rounded-xl border border-border">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={10} className="p-8 text-center text-muted-foreground">Kayıt bulunamadı.</td>
                  </tr>
                )}
              </tbody>
            </table>
