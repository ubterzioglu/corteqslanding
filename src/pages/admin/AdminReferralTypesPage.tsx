...
const AdminReferralTypesPage = () => {
  ...
  const deleteType = async (id: string) => {
    try {
      const updated = await updateReferralType({ id, is_active: false });
      setItems((current) => current.map((item) => (item.id === id ? updated : item)));
      toast({ title: "Type silindi" });
    } catch (error) {
                ...
                items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="flex gap-2">
                      ...
                      <Button variant="destructive" size="sm" onClick={() => void deleteType(item.id)} disabled={!item.is_active}>
                        Sil
                      </Button>
                    </TableCell>
                  </TableRow>
